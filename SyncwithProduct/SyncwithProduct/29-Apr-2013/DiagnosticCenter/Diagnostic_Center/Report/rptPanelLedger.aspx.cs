...
public partial class Report_rptPanelLedger : System.Web.UI.Page
{
    ...
    private void FillGV()
    {
       // clsBLPatientReg_Inv obj_reginv = new clsBLPatientReg_Inv();
        ...
        DataView dv_pr = report.GetAll(38);
        if (dv_pr.Count > 0)
        {
    ...
    protected void gvMain_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        //{
        //    ((Label)e.Row.Cells[0].FindControl("lblBal")).Text = (Convert.ToDouble(((Label)e.Row.Cells[0].FindControl("lbltotal")).Text) - Convert.ToDouble(((Label)e.Row.Cells[0].FindControl("lblpaid")).Text) - Convert.ToDouble(((Label)e.Row.Cells[0].FindControl("lbldis")).Text)).ToString().Trim();
    ...
    private void AdjustScreenDisplay()
    {
        for (int i = 0; i < gvMain.Rows.Count; i++)
        {
            try
            {
                if (gvMain.DataKeys[i].Values["bookingid"].ToString().Trim() == lastbookingid)
                {
                    (gvMain.Rows[i].FindControl("lbllabid") as Label).Text = "";
                    (gvMain.Rows[i].FindControl("lblBookedOn") as Label).Text = "";
                    (gvMain.Rows[i].FindControl("lblpatName") as Label).Text = "";
                    (gvMain.Rows[i].FindControl("lblTotal") as Label).Text = "";
                    (gvMain.Rows[i].FindControl("lblpaid") as Label).Text = "";
                    (gvMain.Rows[i].FindControl("lbldis") as Label).Text = "";
                    (gvMain.Rows[i].FindControl("lblbal") as Label).Text = "";
                    (gvMain.Rows[i].FindControl("Label1") as Label).Text = "";
                    (gvMain.Rows[i].FindControl("lbldate") as Label).Text = "";

                }
                else
                {
                    lastbookingid = gvMain.DataKeys[i].Values["bookingid"].ToString().Trim();
                    serialno++;
                    (gvMain.Rows[i].FindControl("lbldate") as Label).Text = serialno.ToString() ;
                }
    ...
    private void calctotalcharges()
    {
        ...
        //int totalbal = 0;
        int totbalance = 0;
        double totaPayable = 0;
        
        for (int i = 0; i < gvMain.Rows.Count; i++)
        {
            try
            {
                totaPayable += double.Parse(gvMain.DataKeys[i].Values["LabShare"].ToString().Trim());
        ...
        //for (int i = 0; i < gvMain.Rows.Count; i++)
        //{
        //    GridView gvInvestigations = (GridView)gvMain.Rows[i].Cells[0].FindControl("gvInvestigations");
        //    for (int j = 0; j < gvInvestigations.Rows.Count; j++)
        //    {
        //        totaPayable += double.Parse(gvInvestigations.DataKeys[j].Value.ToString().Trim());
        //    }
        //}
        ...
        lblTotAmountPayable.Text = (Math.Round(totaPayable)).ToString().Trim();//(Math.Round(totaPayable/10)*10.0).ToString().Trim()
    }
}