...
public partial class transaction_Rpt_CashInvoice : System.Web.UI.Page
{
    ...
    private void FillCashierlist()
    {
        clsDayCashCollection obj_reporting = new clsDayCashCollection();
       // clsReporting obj_reporting = new clsReporting();
        ...
        obj_reporting.FromDate1 = Request.QueryString["datefrom"].ToString().Trim();
        obj_reporting.ToDate1 = DateTime.Parse(Request.QueryString["dateto"].ToString().Trim(), new CultureInfo("ur-pk", false)).AddDays(1).ToString("dd/MM/yyyy");
        DataView dv = obj_reporting.GetAll(28);
        DataView dv2 = obj_reporting.GetAll(30);
       
            gvcashierlist.DataSource = dv;
            gvcashierlist.DataBind();
       
          //  DataView dv2 = obj_reporting.GetAll(30);
            GVrunningcashier.DataSource = dv2;
            GVrunningcashier.DataBind();
    ...
    private void FIllPanelInformation()
    {
        ...
        dr["PrintedOn"] = System.DateTime.Now.ToString("dd-MM-yyyy hh:mm:ss").ToString();
        //dr["BranchName"] = "All";
        dt.Rows.Add(dr);
    ...
    private void Calculatetotals()
    {
        ...
        lblTotTodPanel.Text = "0";
            //(Convert.ToDouble(lblGrossPanel.Text) - Convert.ToDouble(lblDisPanel.Text) - Convert.ToDouble(lblCanPanel.Text) - Convert.ToDouble(lblBalPanel.Text)).ToString();//"C", CultureInfo.CreateSpecificCulture("ur-pk")).Replace(".00", "");
        ...
        lblNetPanel.Text = "0";
            //(Convert.ToDouble(lblTotTodPanel.Text) + Convert.ToDouble(lblPreBalPanel.Text)).ToString();
        #endregion
    ...
    protected void gvMain_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            GridView gvInvestigations = (GridView)e.Row.FindControl("gvInvestigations");
