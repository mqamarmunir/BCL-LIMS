...
public partial class Report_rptbal_discsummary : System.Web.UI.Page
{
    ...
    private void FillGV()
    {
        ...
        if (!Request.QueryString["branchid"].ToString().Trim().Equals("-1") && !Request.QueryString["branchid"].ToString().Trim().Equals(""))
        {
            report.BranchID = Request.QueryString["branchid"].ToString().Trim();
            
        }
        lblBranch.Text = Request.QueryString["branchName"].ToString().Trim();
        report.Datefrom = DateTime.Parse(Request.QueryString["datefrom"], new CultureInfo("ur-Pk", false)).ToString("dd/MM/yyyy");
        report.Dateto = DateTime.Parse(Request.QueryString["dateto"], new CultureInfo("ur-Pk", false)).ToString("dd/MM/yyyy");
        DataView dv_pr = report.GetAll(39);
        if (dv_pr.Count > 0)
        {
