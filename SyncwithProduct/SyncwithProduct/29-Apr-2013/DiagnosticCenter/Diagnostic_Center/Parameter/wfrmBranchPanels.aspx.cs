...
using System.Data;
using BusinessLayer;
public partial class Parameter_wfrmBranchPanels : System.Web.UI.Page
{
    ...
    private void FillGV()
    {
        ...
        DataView dv = pn.GetAll(8);

        if (dv.Count > 0)
        {
            divpanels.Visible = true;
            gvPanels.DataSource = dv;
            gvPanels.DataBind();
        }
        else
        {
            divpanels.Visible = false;
            gvPanels.DataSource = null;
            gvPanels.DataBind();
        }
    }
    ...
    {
        clsBLBranchPanels obj_brpanels = new clsBLBranchPanels();
    ...
    private void Insert(string type)
    {
        ...
        if (type == "S")
        {
            foreach (GridViewRow row in gvPanels.Rows)
            {
                if (((CheckBox)gvPanels.Rows[row.RowIndex].Cells[3].FindControl("gvchkselect")).Checked == true)
                {
                    countchecked++;
    ...
    protected void lnkRemoveAll_Click(object sender, EventArgs e)
    {
        ...
        foreach (GridViewRow row in gvSelected.Rows)
        {
            ...
            obj_brpanels.BranchPanelID = gvSelected.DataKeys[row.RowIndex].Values[0].ToString().Trim();
           // obj_brpanels.PanelID = gvPanels.DataKeys[row.RowIndex].Value.ToString().Trim();
            obj_brpanels.BranchID = ddlBranch.SelectedValue.ToString().Trim();
    ...
    {
        clsBLBranchPanels obj_brpanels = new clsBLBranchPanels();
        obj_brpanels.BranchPanelID = gvSelected.DataKeys[e.RowIndex].Values[0].ToString().Trim();
        obj_brpanels.BranchID = ddlBranch.SelectedValue.ToString().Trim();
        obj_brpanels.Active = "N";
       // obj_brpanels.ClientId = ddlOrganiztions.SelectedItem.Value.ToString().Trim();
        obj_brpanels.ClientId = System.Configuration.ConfigurationManager.AppSettings["clientid"].ToString().Trim();
