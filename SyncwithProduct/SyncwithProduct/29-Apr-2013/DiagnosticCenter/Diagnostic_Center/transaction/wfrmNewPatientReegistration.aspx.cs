...
{{document.getElementById('{0}').value;}}", tb.ClientID), true);
    ...
    clsDayCashCollection daycash = new clsDayCashCollection();
        //daycash.Personid1 = Session["personid"].ToString();
        ...
        if (dv7.Count > 0)
        {
            ...
            if (lblStatus.Text == "i")
                ...
                Update();
            //daycash.CashStatus = dv7[0]["cashStatus"].ToString();
            //if (daycash.CashStatus == "Offline")
            //{
            // }
        }
        ...
        else
        {
            ...
            //hyper.NavigateUrl = "wfrmCashierEntry.aspx";
            //hyper.ToolTip = "Go to Cashier Entry";
    ...
    protected void imgPRno_Click(object sender, ImageClickEventArgs e)
    {
        ...
        else
        {
            ...
            if (dv.Count > 0)
            {
                ...
                txtAuthorize.Text = dv[0]["authorizeno"].ToString().Trim();
                ...
                try
                {
                    ...
                    if (dv[0]["cashpanel"].Equals("Y"))
                    {
                        rdbPnl_Cash.Checked = true;
                    }
                    else { rdbPanel.Checked = true; }
    ...
    private void Insert()
    {
        ...
        reg.BranchID = Session["ClientID"].ToString().Trim();// System.Configuration.ConfigurationManager.AppSettings["clientid"].ToString();
        ...
        reg.ClientID = Session["ClientiD"].ToString().Trim();// System.Configuration.ConfigurationManager.AppSettings["clientid"].ToString();
        ...
        if (reg.Patient_Insert())
        {
            ...
            {
                if (GetPanelType(ddlPanel.SelectedValue.ToString()).Equals("Y"))
                {
                    cashpnel = "Y";
                }
                else
            ...
            Response.Redirect("wfrmNewInvestigaionBooking.aspx?prid=" + reg.PRID + " &prno=" + reg.PRNO + " &doctorid=" + this.ddlDoctor.SelectedItem.Value.ToString() + " &referredby=" + this.txtReferredBy.Text.Trim() + " &panelid=" + str_panelID + " &patienttype=" + str_rdb + " &deliveryby=" + ddlDeliveryMode.SelectedItem.Value.ToString() + " &age=" + GetAge(reg.DOB) + " &patient=" + ddlTitle.SelectedItem.Text.Trim().Replace("Select", "") + " " + this.txtName.Text.Trim() + " &gender=" + reg.Gender + " &marital=" + ddlMarital.SelectedItem.Text.Trim() + " &authorizeno=" + str_Authorize + " &panelname=" + str_panelName + " &referenceno=" + txtReferenceNo.Text.Trim().Replace("#", "%%").Replace("&", ">>") + " &remarks=" + txtRemark.Text.Trim() + " &CliqueId="+Rot13.Transform(txtPathCliqueiD.Text.Trim())+"&cashpanel="+cashpnel);
        }
        else
        {
            lblError.ForeColor = System.Drawing.Color.Red;
    ...
    private void Update()
    {
        ...
        reg.HPhone = this.txtHPHone.Text.Trim();
        reg.CellNo = this.txtCellNo.Text.Trim();
        reg.Password = txtName.Text.Trim().Split(' ').GetValue(0).ToString();
        reg.Fax = this.txtFax.Text.Trim();
        reg.Email = this.txtEmail.Text.Trim();
    ...
    private void Dependent_Update()
    {
        ...
        reg.EnteredBy = Session["personid"].ToString();
        reg.EnteredOn = System.DateTime.Now.ToString("dd/MM/yyyy hh:mm:ss tt");
        reg.ClientID = Session["Clientid"].ToString().Trim();//Session["orgid"].ToString(); // org
    ...
    private bool Validate_Form()
    {
        ...
        if (rdbPanel.Checked == true || rdbPnl_Cash.Checked == true)
        {
            ...
            //    return false;
            //}
            if (txtAuthorize.Text.Trim().Equals("") && rdbPanel.Checked)
            {
    ...
    private void FillDoctor_DDL()
    {
        ...
        //com.FillComboBox(cmbReferredBy, dv, "name", "doctorid");
    }
    ...
    private string GetPanelType(string id)
    {
        string type = "";
        clsBLPatientReg_Inv c = new clsBLPatientReg_Inv();
        c.PanelID = ddlPanel.SelectedValue.ToString();
        DataView dv = c.GetAll(57);
        if (dv[0]["cashpanel"].Equals("Y"))
        {
            type = "Y";
        }
    ...
    protected void gvPatient_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName.Equals("Select"))
        {
            ...
            txtAuthorize.Text = gvPatient.DataKeys[index].Values["authorizeno"].ToString().Trim();
            try
            {
                ddlMarital.SelectedItem.Selected = false;
            ...
            try
            {
                ...
                try
                {
                     cashpnl = gvPatient.DataKeys[index].Values[14].ToString();
                }
                catch { }
                rdbCash.Checked = false;
             
                pnlPanel.Visible = true;
                if (!string.IsNullOrEmpty(cashpnl) && !cashpnl.Equals("N"))
                {
                    rdbCash.Checked = false;
                    rdbPanel.Checked = false;
                    rdbPnl_Cash.Checked = true;
    ...
    protected void imgcellnosearch_Click(object sender, ImageClickEventArgs e)
    {
        lblError.ForeColor = System.Drawing.Color.Red;
        lblError.Text = "";
        if (txtCellNo.Text.Trim().Equals(""))
        {
            lblError.Text = "Please enter Cell No";
            return;
        }
        else
        {
            ...
            if (dv.Count > 0)
            {
                ...
                txtPRnO.Text = dv[0]["PRNo"].ToString().Trim();
                txtName.Text = dv[0]["name"].ToString();
