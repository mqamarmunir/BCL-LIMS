...
{
    /// <summary>
    /// Drop down check box list
    /// </summary>
    /// <remarks>
    /// Drop down list of check boxes. The control is inherited from CheckBoxList control has the same capabilities as teh parent. The only difference is in rendering and styling options.
    /// </remarks>
    public class DropDownCheckBoxes : CheckBoxList, IPostBackEventHandler
    {
        #region Constructor

        public DropDownCheckBoxes()
        {
        ...
        protected override void Render(System.Web.UI.HtmlTextWriter writer)
        {
            ...
            if (UseButtons)
            {
                ...
                var tag = string.Format(inputTag, Texts.OkButton);
                writer.Write(tag + "&nbsp;");

                tag = string.Format(inputTag, Texts.CancelButton);
                writer.Write(tag);
