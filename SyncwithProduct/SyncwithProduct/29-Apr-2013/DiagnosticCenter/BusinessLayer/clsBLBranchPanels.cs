...
{
    public class clsBLBranchPanels
    {
        public clsBLBranchPanels()
        {
        ...
        private const string TableName = "dc_tp_BranchPanels";

        private string StrErrorMessage = "";

        private string _BranchPanelID = Default;
        ...
        #region Properties
        public string BranchPanelID
        {
            get { return _BranchPanelID; }
            set { _BranchPanelID = value; }
        }
        ...
        public DataView GetAll(int flag)
        {
            ...
            {
                case 1:
                    objdbhims.Query = "Select bp.branchPanelID,bp.branchID,bp.panelID,p.Name PanelName From "+TableName +" bp inner join dc_tpanel p on p.panelid=bp.panelid where bp.Active='Y' and bp.branchID="+_BranchID;
                    break;
                case 2:
                    objdbhims.Query = "Select * From " + TableName + " where BranchID=" + _BranchID + " and PanelID=" + _PanelID + " and Active='Y'";
                    break;
            }
        ...
        public bool Insert()
        {
            if (Validation() && Vd_chkduplicate())
            {
                try
                {
                    ...
                    objdbhims.Query = objQB.QBGetMax("BranchPanelID", TableName);
                    this._BranchPanelID = objTrans.DataTrigger_Get_Max(objdbhims);

                    if (!this._BranchPanelID.Equals("True"))
                    {
        ...
        private bool Delete()
        {
            objdbhims.Query = "Update dc_tp_branchPanels set Active='N',EnteredBy=" + Convert.ToInt32(_Enteredby) + " where BranchpanelID=" + Convert.ToInt32(_BranchPanelID);
            objTrans.Start_Transaction();
        ...
        private string[,] MakeArray()
        {
            ...
            if (!this._BranchPanelID.Equals(Default))
            {
                arySAPS[0, 0] = "BranchPanelID";
                arySAPS[0, 1] = this._BranchPanelID;
                arySAPS[0, 2] = "int";
            }
