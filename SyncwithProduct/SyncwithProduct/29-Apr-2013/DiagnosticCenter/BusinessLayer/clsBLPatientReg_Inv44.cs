    ...
    public string History_antobiotics
    {
        ...
        public DataView GetAll(int flag)
        {
            switch (flag)
            {
                ...
                case 6: // fill panel
                    objdbhims.Query = "select panelid,concat(name,' (',case when clientid = '006' then 'BCL' when clientid = '008' then 'AMC' else '-' end ,')') as name from dc_tpanel where active='Y' and clientid in (select allow_panel_org from dc_tp_org_allowpanel where orgid='" + StrClientID + "' ) and panelid in (Select panelid From dc_tp_branchpanels where branchid="+_BranchID+" and Active='Y') order by clientid";
                ...
                case 32:
                    ...
                    break;
                case 33:
                    objdbhims.Query = "SELECT name,gender,dob,hphone,Pasword,enteredon FROM dc_tpatient WHERE prid = '3' ";
                    break;
                case 34:
                ...
                case 41: //fill branch tests w.r.t panel patient
                    objdbhims.Query = @"select  c.classificationid, t.testid,bt.TestPrice branchcost,
                                        ...
                                        left outer join dc_tp_method mt on t.d_methodid=mt.methodid
                                        ...
                                        where t.active='Y' and ag.maxage >" + Convert.ToInt32(StrDOB) + @"
                ...
                case 42:
                   // objdbhims.Query = "SELECT Name,ifnull(StreetAddress,'Not Available') as address FROM dc_tp_branch d WHERE Active = 'Y'";
                    objdbhims.Query = "SELECT ifnull(b.starttime,'00:00') as starttime,ifnull(b.endtime,'00:00') as endtime,b.printreporttext,ifnull(b.Name,'Bio Care Laborartory') as branchName,b.Report_Text,ifnull(b.StreetAddress,'Not Available') as address,CONCAT('(',b.PhoneNo,')') as phone,ifnull(c.Name,'-') as city FROM dc_tp_branch b , dc_tcity c WHERE c.cityid = b.city AND b.printreporttext='Y'";
                    break;
                case 43:
                ...
                case 44:
                    ...
                    break;
                case 45:
                    objdbhims.Query = "SELECT PrintBranches,PrintTestComents,ifnull(GeneralComments,'No Comments available') as coments FROM dc_tp_preferencesettings";
                    break;
                ...
                case 48:
                    objdbhims.Query = @"select m.bookingid, m.totalamount as charges,concat(ifnull(pp.title,''),' ',
                                        ...
                                        end as gender,m.bookingid,m.prid,pn.panelid,
                                        ...
                                        case when DATEDIFF(CURRENT_DATE,pp.dob) > 365 then      concat(Floor(DATEDIFF(CURRENT_DATE,pp.dob)/365),' (Y)')  else
                                         case when DATEDIFF(CURRENT_DATE,pp.dob) > 30 then      concat(Floor(DATEDIFF(CURRENT_DATE,pp.dob)/30),' (M)')    else
                                             ...
                                             pn.print_amount,w.name as ward,m.type as ind_outdoor,m.labid,m.enteredon as bookedon,
                    ...
                    break;
                case 49:
                    objdbhims.Query = @"Select d.bookingid,t.test_name
                                        From dc_tpatient_bookingd d inner join dc_tp_test t on t.testid=d.testid
                           ...
                           // StrDelivery_Time = ts.ToString("dd/MM/yyyy hh:mm:ss tt");
                            ...
                            if (dv_dplan.Count > 0)
                            {
                                if (dv_dplan[0]["dplan"].ToString().Trim() == "Y")
                                {
                                    ...
                                    {
                                        if (DateTime.Parse(System.DateTime.Now.ToString("dd/MM/yyyy hh:mm:ss tt"), new CultureInfo("en-GB", false)) <= DateTime.Parse(System.DateTime.Now.ToString("dd/MM/yyyy") + " " + dv_dplan[0]["time1"].ToString().Trim(), new CultureInfo("en-GB", false)))
                                        {
                                    ...
                                    {
                                        if (DateTime.Parse(System.DateTime.Now.ToString("dd/MM/yyyy hh:mm:ss tt"), new CultureInfo("en-GB", false)) <= DateTime.Parse(System.DateTime.Now.ToString("dd/MM/yyyy") + " " + dv_dplan[0]["time2"].ToString().Trim(), new CultureInfo("en-GB", false)))
                                        {
                                    ...
                                    {
                                        if (DateTime.Parse(System.DateTime.Now.ToString("dd/MM/yyyy hh:mm:ss tt"), new CultureInfo("en-GB", false)) <= DateTime.Parse(System.DateTime.Now.ToString("dd/MM/yyyy") + " " + dv_dplan[0]["time3"].ToString().Trim(), new CultureInfo("en-GB", false)))
                                        {
                                    ...
                                    {
                                        if (DateTime.Parse(System.DateTime.Now.ToString("dd/MM/yyyy hh:mm:ss tt"), new CultureInfo("en-GB", false)) <= DateTime.Parse(System.DateTime.Now.ToString("dd/MM/yyyy") + " " + dv_dplan[0]["time4"].ToString().Trim(), new CultureInfo("en-GB", false)))
                                        {
                                    ...
                                    if (flag == 0)
                                    {
                                        ...
                                        StrDelivery_Time = dtime.ToString("dd/MM/yyyy") + " " + dv_dplan[0]["dispatchtime1"].ToString().Trim();
                                    }
                                }
                                else
                                {
        ...
        private string[,] MakeArray_Patient()
        {
            ...
            {
                kd_arr[22, 0] = "BranchID";
                kd_arr[22, 1] = this._BranchID;
                kd_arr[22, 2] = "int";
            }
            return kd_arr;
        }
        ...
        {
            string[,] kd_array = new string[25, 3];
